...
RESULTS_DIR   = os.path.join(AI_DIR, "results", "experiment9")

EXPERIMENT_NAME = "experiment9_label_smoothing"
CHECKPOINT_NAME = "experiment9_best_model.pt"
...
# -- Baseline Loaders for Experiments 1 through 8 -------------------------------
BASELINES = {
    f"exp{i}": {
        "name": f"Experiment {i}",
...
def run_test_evaluation(model, device, test_df, results_dir, checkpoint_path):
    ...
    df_idx    = CLASS_MAPPING['df']
    vasc_idx  = CLASS_MAPPING['vasc']
    ...
    mel_to_akiec_errors = int(cm[mel_idx][akiec_idx])
    mel_to_bcc_errors = int(cm[mel_idx][bcc_idx])
    mel_to_df_errors = int(cm[mel_idx][df_idx])
    mel_to_vasc_errors = int(cm[mel_idx][vasc_idx])

    # Total and false-positive melanoma predictions
    mel_pred_total = int(sum(all_preds[i] == mel_idx for i in range(len(all_preds))))
    mel_fp_total   = mel_pred_total - mel_correct
    ...
    print(f"  Total Mel Predictions  : {mel_pred_total} (False Positives: {mel_fp_total})")
    print(f"  mel -> nv errors       : {mel_to_nv_errors} images")
    print(f"  mel -> bkl errors      : {mel_to_bkl_errors} images")
    print(f"  mel -> akiec errors    : {mel_to_akiec_errors} images")
    print(f"  mel -> bcc errors      : {mel_to_bcc_errors} images")
    print(f"  mel -> df errors       : {mel_to_df_errors} images")
    print(f"  mel -> vasc errors     : {mel_to_vasc_errors} images")
    ...
    with open(os.path.join(results_dir, "classification_report.txt"), "w") as f:
        f.write("EXPERIMENT 9 -- MILD LABEL SMOOTHING (0.05) WITH TEMPERED CLASS WEIGHTS (alpha=0.75)\n")
        f.write("Test Set Evaluation (Held-Out Test Split -- 988 Images)\n")
        f.write(f"{'='*60}\n")
        f.write(f"Backbone          : MobileNetV3-Large + CBAM Attention\n")
        f.write(f"Loss Strategy     : Tempered Class-Weighted CE (alpha=0.75) + Label Smoothing (0.05)\n")
        ...
        f.write(f"Total Mel Preds   : {mel_pred_total} (False Positives: {mel_fp_total})\n")
        f.write(f"mel -> nv errors  : {mel_to_nv_errors} images\n")
        f.write(f"mel -> bkl errors : {mel_to_bkl_errors} images\n")
        f.write(f"mel -> akiec errs : {mel_to_akiec_errors} images\n")
        f.write(f"mel -> bcc errs   : {mel_to_bcc_errors} images\n")
        f.write(f"mel -> df errs    : {mel_to_df_errors} images\n")
        f.write(f"mel -> vasc errs  : {mel_to_vasc_errors} images\n\n")
    ...
    metrics = {
        ...
        "melanoma_false_positives": mel_fp_total,
        "melanoma_to_nv_errors": mel_to_nv_errors,
        "melanoma_to_bkl_errors": mel_to_bkl_errors,
        "melanoma_to_akiec_errors": mel_to_akiec_errors,
        "melanoma_to_bcc_errors": mel_to_bcc_errors,
        "melanoma_to_df_errors": mel_to_df_errors,
        "melanoma_to_vasc_errors": mel_to_vasc_errors,
    ...
    plt.title('Confusion Matrix -- Test Set (Experiment 9: Label Smoothing 0.05 + Tempered CE)')
    plt.tight_layout()
...
# -- Comparison Helper Across All Prior Experiments (1 through 8) ---------------
def compute_comparisons(exp9_metrics, results_dir):
    comparisons = {}
    for exp_key in [f"exp{i}" for i in range(1, 9)]:
        base = load_baseline_metrics(exp_key)
        diff = {
            "accuracy_change": float(exp9_metrics["accuracy"] - base["accuracy"]) if base.get("accuracy") is not None else None,
            "balanced_accuracy_change": float(exp9_metrics["balanced_accuracy"] - base["balanced_accuracy"]) if base.get("balanced_accuracy") is not None else None,
            "macro_f1_change": float(exp9_metrics["macro_f1"] - base["macro_f1"]) if base.get("macro_f1") is not None else None,
            "macro_precision_change": float(exp9_metrics["macro_precision"] - base["macro_precision"]) if base.get("macro_precision") is not None else None,
            "macro_recall_change": float(exp9_metrics["macro_recall"] - base["macro_recall"]) if base.get("macro_recall") is not None else None,
            "weighted_f1_change": float(exp9_metrics["weighted_f1"] - base["weighted_f1"]) if base.get("weighted_f1") is not None else None,
            "melanoma_recall_change": float(exp9_metrics["melanoma_recall"] - base["melanoma_recall"]) if base.get("melanoma_recall") is not None else None,
            "melanoma_precision_change": float(exp9_metrics["melanoma_precision"] - base["melanoma_precision"]) if base.get("melanoma_precision") is not None else None,
            "melanoma_f1_change": float(exp9_metrics["melanoma_f1"] - base["melanoma_f1"]) if base.get("melanoma_f1") is not None else None,
            "mel_to_nv_error_change": int(exp9_metrics["melanoma_to_nv_errors"] - base["mel_to_nv_errors"]) if (base.get("mel_to_nv_errors") is not None and exp9_metrics.get("melanoma_to_nv_errors") is not None) else None,
            "mel_to_bkl_error_change": int(exp9_metrics["melanoma_to_bkl_errors"] - base["mel_to_bkl_errors"]) if (base.get("mel_to_bkl_errors") is not None and exp9_metrics.get("melanoma_to_bkl_errors") is not None) else None,
            "nv_recall_change": float(exp9_metrics["nv_recall"] - base["nv_recall"]) if base.get("nv_recall") is not None else None,
            "bcc_recall_change": float(exp9_metrics["bcc_recall"] - base["bcc_recall"]) if base.get("bcc_recall") is not None else None,
            "bkl_recall_change": float(exp9_metrics["bkl_recall"] - base["bkl_recall"]) if base.get("bkl_recall") is not None else None,
        }
        ...
        comp_data = {
            ...
            "experiment9_label_smoothing": {
                ...
                "balanced_accuracy": exp9_metrics["balanced_accuracy"],
                "macro_precision": exp9_metrics["macro_precision"],
                "macro_recall": exp9_metrics["macro_recall"],
                "macro_f1": exp9_metrics["macro_f1"],
                "weighted_f1": exp9_metrics["weighted_f1"],
                "melanoma_recall": exp9_metrics["melanoma_recall"],
                "melanoma_precision": exp9_metrics["melanoma_precision"],
                "melanoma_f1": exp9_metrics["melanoma_f1"],
                "mel_to_nv_errors": exp9_metrics["melanoma_to_nv_errors"],
                "mel_to_bkl_errors": exp9_metrics["melanoma_to_bkl_errors"],
                "nv_recall": exp9_metrics["nv_recall"],
                "bcc_recall": exp9_metrics["bcc_recall"],
                "bkl_recall": exp9_metrics["bkl_recall"],
            },
            f"delta_exp9_minus_{exp_key}": diff
        }
    ...
    # Print comparison against primary baseline (Experiment 8)
    exp8_diff = comparisons["exp8"]["delta_exp9_minus_exp8"]
    exp8_base = comparisons["exp8"]["baseline"]
    print("\n" + "="*60)
    print("COMPARISON: EXPERIMENT 9 vs EXPERIMENT 8 (PRIMARY BASELINE)")
    print("="*60)
    print(f"  Test Accuracy      : {exp8_base['accuracy']*100:.2f}% -> {exp9_metrics['accuracy']*100:.2f}%  (delta: {exp8_diff['accuracy_change']*100:+.2f}%)")
    print(f"  Balanced Accuracy  : {exp8_base['balanced_accuracy']*100:.2f}% -> {exp9_metrics['balanced_accuracy']*100:.2f}%  (delta: {exp8_diff['balanced_accuracy_change']*100:+.2f}%)")
    print(f"  Macro Precision    : {exp8_base['macro_precision']:.4f} -> {exp9_metrics['macro_precision']:.4f}  (delta: {exp8_diff['macro_precision_change']:+.4f})")
    print(f"  Macro Recall       : {exp8_base['macro_recall']:.4f} -> {exp9_metrics['macro_recall']:.4f}  (delta: {exp8_diff['macro_recall_change']:+.4f})")
    print(f"  Macro F1           : {exp8_base['macro_f1']:.4f} -> {exp9_metrics['macro_f1']:.4f}  (delta: {exp8_diff['macro_f1_change']:+.4f})")
    print(f"  Weighted F1        : {exp8_base['weighted_f1']:.4f} -> {exp9_metrics['weighted_f1']:.4f}  (delta: {exp8_diff['weighted_f1_change']:+.4f})")
    print(f"  Melanoma Recall    : {exp8_base['melanoma_recall']*100:.2f}% -> {exp9_metrics['melanoma_recall']*100:.2f}%  (delta: {exp8_diff['melanoma_recall_change']*100:+.2f}%)")
    print(f"  Melanoma Precision : {exp8_base['melanoma_precision']:.4f} -> {exp9_metrics['melanoma_precision']:.4f}  (delta: {exp8_diff['melanoma_precision_change']:+.4f})")
    print(f"  Melanoma F1        : {exp8_base['melanoma_f1']:.4f} -> {exp9_metrics['melanoma_f1']:.4f}  (delta: {exp8_diff['melanoma_f1_change']:+.4f})")
    mel_nv_d = exp8_diff.get('mel_to_nv_error_change')
    mel_nv_str = f"{mel_nv_d:+d}" if mel_nv_d is not None else "N/A"
    mel_bkl_d = exp8_diff.get('mel_to_bkl_error_change')
    mel_bkl_str = f"{mel_bkl_d:+d}" if mel_bkl_d is not None else "N/A"
    print(f"  Mel -> nv errors   : {exp8_base.get('mel_to_nv_errors', 'N/A')} -> {exp9_metrics['melanoma_to_nv_errors']}  (delta: {mel_nv_str})")
    print(f"  Mel -> bkl errors  : {exp8_base.get('mel_to_bkl_errors', 'N/A')} -> {exp9_metrics['melanoma_to_bkl_errors']}  (delta: {mel_bkl_str})")
    print(f"  Majority (nv) Rec  : {exp8_base['nv_recall']*100:.2f}% -> {exp9_metrics['nv_recall']*100:.2f}%  (delta: {exp8_diff['nv_recall_change']*100:+.2f}%)")
    if exp8_base.get('bcc_recall') is not None and exp8_diff.get('bcc_recall_change') is not None:
        print(f"  BCC Recall         : {exp8_base['bcc_recall']*100:.2f}% -> {exp9_metrics['bcc_recall']*100:.2f}%  (delta: {exp8_diff['bcc_recall_change']*100:+.2f}%)")
    if exp8_base.get('bkl_recall') is not None and exp8_diff.get('bkl_recall_change') is not None:
        print(f"  BKL Recall         : {exp8_base['bkl_recall']*100:.2f}% -> {exp9_metrics['bkl_recall']*100:.2f}%  (delta: {exp8_diff['bkl_recall_change']*100:+.2f}%)")
    print("="*60)
    return comparisons
...
def train():
    ...
    # 3. Loss with Native PyTorch Label Smoothing (The ONLY experimental variable in Exp 9)
    criterion = nn.CrossEntropyLoss(weight=weights_tensor, label_smoothing=LABEL_SMOOTHING)

    # Save class weights, loss configuration, and tempering configuration
    ...
    loss_config = {
        ...
        "class_weights_strategy": "tempered_normalized_training_only",
        "alpha": ALPHA,
        "normalized_weights": norm_weights_dict,
        "reduction": "mean",
        "difference_from_experiment8": "Added label_smoothing=0.05; all weights and other hyperparams identical"
    }
    with open(os.path.join(RESULTS_DIR, "loss_configuration.json"), "w") as f:
        json.dump(loss_config, f, indent=4)

    # 4. Datasets and Loaders (keeping identical to Exp 8: natural random sampling, identical augmentation)
    ...
    # 5. Model Architecture: MobileNetV3-Large + CBAM Attention (Exact Exp 8 Architecture)
    ...
    # 6. Pre-Training Verification (Requirement 9)
    print("="*60)
    ...
    print(f"  Phase 1 Trainable Parameters      : {trainable_phase1:,}  (Backbone frozen, Epochs 1-15)")
    print(f"  Phase 2 Trainable Parameters      : {trainable_phase2:,}  (Full network, Epochs 16-30)")
    print(f"  Dataset Partition Sizes           : Train={len(train_df)}, Val={len(val_df)}, Test={len(test_df)}")
    print(f"  Random Seed                       : {SEED}")
    print(f"  Batch Size                        : {BATCH_SIZE}")
    print(f"  Tempering Exponent (alpha)        : {ALPHA}")
    print(f"  Label Smoothing                   : {LABEL_SMOOTHING}  (THE ONLY EXPERIMENTAL CHANGE)")
    print(f"  Normalized Tempered Weights       : {norm_weights_dict}")
    ...
    print(f"  Verification Check                : Experiment 9 differs from Experiment 8 ONLY by label_smoothing={LABEL_SMOOTHING}")
    print("="*60 + "\n")
    ...
    # 7. Optimizer & Scheduler (Identical to Experiment 8)
    ...
    # 8. Smoke Test (Requirement 10)
    print("="*60)
    print("SMOKE TEST (REQUIREMENT 10)")
    print("="*60)
    print("[SMOKE TEST] Fetching one batch from train_loader ...")
    try:
        smoke_imgs, smoke_labels = next(iter(train_loader))
        smoke_imgs, smoke_labels = smoke_imgs.to(device), smoke_labels.to(device)
        print(f"[SMOKE TEST] Image tensor shape : {smoke_imgs.shape}")
        print(f"[SMOKE TEST] Label tensor shape : {smoke_labels.shape}")

        optimizer.zero_grad()
        smoke_outputs = model(smoke_imgs)
        smoke_loss = criterion(smoke_outputs, smoke_labels)
        print(f"[SMOKE TEST] Loss computed       : {smoke_loss.item():.4f}")

        # Check for NaN
        if torch.isnan(smoke_loss).item():
            raise ValueError("[SMOKE TEST] Loss is NaN!")

        smoke_loss.backward()

        # Verify gradient flow to trainable parameters
        grad_norms = [p.grad.norm().item() for p in model.parameters() if p.requires_grad and p.grad is not None]
        if not grad_norms:
            raise ValueError("[SMOKE TEST] No gradients computed for trainable parameters!")

        optimizer.step()
        optimizer.zero_grad()
        print(f"[SMOKE TEST] Gradients verified  : {len(grad_norms)} trainable tensors updated (mean norm={np.mean(grad_norms):.4f})")
        print("[SMOKE TEST] PASSED -- proceeding to full training.\n")
        del smoke_imgs, smoke_labels, smoke_outputs, smoke_loss
    ...
    # 9. Training Loop (Exact Experiment 8 2-Phase Schedule)
    ...
    print(f"Starting Experiment 9 training for up to {EPOCHS} epochs with label smoothing 0.05...")
    print("="*60)
    ...
    # 10. Save history & generate training curves
    ...
    plt.suptitle('Experiment 9 -- Label Smoothing (0.05) + Tempered CE Training Curves', fontsize=14)
    plt.tight_layout()
    ...
    # 11. Single Test Evaluation
    metrics = run_test_evaluation(model, device, test_df, RESULTS_DIR, checkpoint_path)

    # 12. Comparisons against all prior experiments (1 through 8)
    comparisons = compute_comparisons(metrics, RESULTS_DIR)

    # 13. Experiment Summary & Reproducibility Metadata
    reproducibility = {
        ...
        "scheduler": f"ReduceLROnPlateau(mode=min, factor=0.1, patience={PATIENCE})",
        "model_name": "DermaAI_MobileNetV3",
        "pretrained_weights": "MobileNet_V3_Large_Weights.DEFAULT",
        "total_parameters": total_params,
    ...
    summary = {
        ...
        "config": {
            ...
            "batch_size": BATCH_SIZE,
            "loss": "tempered_class_weighted_cross_entropy_with_label_smoothing",
            "alpha": ALPHA,
            "label_smoothing": LABEL_SMOOTHING,
        ...
        "primary_comparison_exp8": comparisons["exp8"],
    ...
    print(f"{'='*60}")
    print("\nExperiment 9 finished. STOPPED -- awaiting next instruction.\n")
